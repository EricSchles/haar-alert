...
parser.add_argument('-sM','--send-message',default=False,help='set to Flase in order to turn off email alerts')
parser.add_argument('-M','--mark-files',default=False,help='set to False in order to turn off file marking.')
...
parser.add_argument('-l','--loop',default='-1',help='continually loop over the given directory at a given interval"-l <seconds-to-wait>"')
...
while True:
	...
	for infile in glob.glob( os.path.join(path, '*.'+args['extension']) ):
		...
		if infile[-extSearchPosition:] == 'hscanned.'+args['extension']:
			...
			if args['display'] == 'True':
				cv.ShowImage('haar-alert viewer',image)
